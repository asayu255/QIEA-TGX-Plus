...
def EA(explainer,model,up_down,factual,lists,events,j,existed,config,pg_index,seed,agents_iteration,_flag):
    ...
    # QIEA-TGX+ is optional. When disabled, the original public QIEA-TGX
    # path, parameters, inputs, and outputs are left unchanged.
    plus_config = config.get('qiea_tgx_plus', None)
    plus_enabled = plus_config is not None and bool(plus_config.get('enabled', False))

    if plus_enabled:
        from functions.QIEAplus_func import Quantum_individual_optimizationA as Quantum_individual_optimizationA_plus

        plus_agents = int(plus_config.get('num_agents', 2))
        if plus_agents < 2 or plus_agents % 2 != 0:
            raise ValueError('QIEA-TGX+ requires an even num_agents value of at least 2.')
        plus_agent_iteration = [plus_agents, int(config.n_trials / plus_agents)]

        result,result_list,loop_list,model_time = Quantum_individual_optimizationA_plus(
            plus_agent_iteration,model, input_data, candidates, H_gate, teta,
            factual, existed,max_events,initial_output,flag,hops,
            theta_s_min=float(plus_config.get('theta_s_min', 0.001)),
            theta_s_max=float(plus_config.get('theta_s_max', 0.01)),
            theta_l_min=float(plus_config.get('theta_l_min', 0.1)),
            theta_l_max=float(plus_config.get('theta_l_max', 0.5)),
            normalization_factor=float(plus_config.get('normalization_factor', 1.0)),
            exchange_interval=int(plus_config.get('exchange_interval', 10)),
            max_exchange_extension=int(plus_config.get('max_exchange_extension', 50)),
            large_stagnation_patience=int(plus_config.get('large_stagnation_patience', 80)),
            stop_stagnation_patience=int(plus_config.get('stop_stagnation_patience', 150)),
            tau_low=float(plus_config.get('tau_low', 0.1)),
            tau_high=float(plus_config.get('tau_high', 0.9)),
        )
    else:
        result,result_list,loop_list,model_time = Quantum_individual_optimizationA(
            agents_iteration,model, input_data, candidates, H_gate, teta,
            factual, existed,max_events,initial_output,flag,hops
        )
...
def pipeline(config: DictConfig):
    ...
    # Keep base and Plus results separate so running qiea and qiea-plus in the
    # same workflow never overwrites or mixes their output files.
    plus_config = config.get('qiea_tgx_plus', None)
    plus_enabled = plus_config is not None and bool(plus_config.get('enabled', False))
    method = "QIEA-TGX+" if plus_enabled else "QIEA-TGX"
