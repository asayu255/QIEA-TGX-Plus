...
class PGExplainerExt(BaseExplainerTG):
    ...
    def __call__(self, node_idxs: Union[int, None] = None, event_idxs: Union[int, None] = None, tgt_src: Union[int, None] = None, tgt_dst: Union[int, None] = None, sparsity: Union[float, None] = None,factual: Union[bool, None] = None,existed: Union[bool, None] = None):
        ...
        if not self.explainer_ckpt_path.exists():
            raise FileNotFoundError(
                f'PGExplainer checkpoint not found: {self.explainer_ckpt_path}. '
                'Run the PGExplainer workflow once to train it first.'
            )
        else:
    ...
    def _obtain_train_idxs(self,):
        size = 1000
        if self.dataset_name in ['wikipedia', 'reddit', 'movielens', 'enron']:
