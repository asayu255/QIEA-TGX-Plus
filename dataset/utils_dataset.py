...
def k_hop_temporal_subgraph(df, num_hops, event_idx):
    ...
    subsets = [[center_node], ]
    # Sources and destinations share one ID space in a homogeneous graph, so
    # the largest node id is not necessarily on the destination side.
    num_nodes = int(max(df_new.iloc[:, 0].max(), df_new.iloc[:, 1].max())) + 1
    ...
    for _ in range(num_hops):
        ...
        # Follow an event from either endpoint. A bipartite graph keeps only
        # the source side anyway, because its two ID ranges are disjoint; a
        # homogeneous graph would otherwise miss every incoming interaction.
        edge_mask = node_mask[source_nodes] | node_mask[target_nodes]
        new_nodes = np.concatenate([target_nodes[edge_mask], source_nodes[edge_mask]])
