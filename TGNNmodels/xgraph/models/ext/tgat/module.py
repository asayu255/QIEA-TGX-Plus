...
class AttnModel(torch.nn.Module):
    ...
    def forward(self, src, src_t, seq, seq_t, seq_e, mask):
        ...
        # The query has no incoming edge; the placeholder must match the edge
        # width, which is not always the node width.
        src_e_ph = src_ext.new_zeros(src_ext.size(0), 1, self.edge_dim)
...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder: NeighborFinder, n_feat, e_feat, device='cuda:0',
        ...
        # Edge features may be narrower than node features (MovieLens ships one
        # rating per event). AttnModel already takes the edge width separately.
